...
class SchoolFeeWaiverDeduction(models.Model):
    """
    Represents the accounting realization of a fee waiver.

    A fee waiver (``school_fee_waiver``) only plans how much of a
    student's billed fees will be waived, term by term, through its
    own Schedule lines. This document is what actually turns that
    plan into an accounting fact: opening it books its own
    ``account.move`` -- crediting the student's Receivable Account on
    the header itself, and debiting a discount/contra-revenue Account
    per Deduction Line -- then reconciles that credit against one or
    more open ``customer_invoice`` documents (its Allocation lines),
    so those invoices' own ``amount_residual`` drops and they move to
    Paid once fully covered. This is the reverse of
    ``customer_invoice``'s own accounting entry: there, the header
    debits Receivable; here, it credits it. Opening this document
    also flips every deducted Schedule line of the linked waiver from
    ``scheduled`` to ``realized``; cancelling it reverses both the
    reconciliation and that flip, in full or not at all.
    """
    ...
    _description = "School Fee Waiver Deduction"
    ...
    waiver_id = fields.Many2one(
        ...
        help="Fee waiver document this deduction realizes.",
    )
    student_id = fields.Many2one(
        ...
        store=True,
        compute_sudo=True,
        help="Student of the selected Waiver.",
    )
    partner_id = fields.Many2one(
        ...
        store=True,
        compute_sudo=True,
        help="The student's contact partner, copied from the selected "
        "Waiver. Must match the Partner of every allocated invoice.",
    )
    ...
    receivable_account_id = fields.Many2one(
        ...
        "amount. Should match the Receivable Account of every "
        "allocated invoice, or reconciliation fails when this "
        "document is opened.",
    )
    line_ids = fields.One2many(
        ...
        help="Deduction lines, one per realized Schedule line. Their "
        "sum is this document's Amount Total.",
    )
    allocation_ids = fields.One2many(
        string="Allocations",
        comodel_name="school_fee_waiver_deduction_allocation",
        inverse_name="deduction_id",
        readonly=True,
        states={
            "draft": [
    ...
    schedule_ids = fields.One2many(
        ...
        inverse_name="deduction_id",
        readonly=True,
        help="Waiver Schedule lines realized by this document. Filled "
    ...
    amount_total = fields.Monetary(
        ...
        store=True,
        compute_sudo=True,
        help="Sum of the Deduction lines' Amount. This is also the "
        "amount credited to the Receivable Account.",
    )
    amount_allocated = fields.Monetary(
        ...
        store=True,
        compute_sudo=True,
        help="Sum of the Allocation lines' Amount.",
    )
    ...
    def _compute_student_id(self):
        """Derive the Student from the selected Waiver.

        :return: nothing; assigns ``student_id``
        """
        for record in self:
            record.student_id = record.waiver_id.student_id

    @api.depends("waiver_id")
    def _compute_partner_id(self):
        """Derive the Partner from the selected Waiver.

        :return: nothing; assigns ``partner_id``
        """
        for record in self:
            record.partner_id = record.waiver_id.partner_id
    ...
    def _compute_amount_allocated(self):
        ...
        """
        for record in self:
            record.amount_allocated = sum(record.allocation_ids.mapped("amount"))
    ...
    def onchange_journal_id(self):
        """Default Journal from the Waiver's own Type.

        :return: nothing
        """
        self.journal_id = False
        if self.waiver_id:
            self.journal_id = self.waiver_id.type_id.deduction_journal_id

    @api.constrains("waiver_id")
    def _check_waiver_state(self):
        """Require the selected Waiver to be Open or Done.

        :raises ValidationError: when ``waiver_id.state`` is neither
            ``open`` nor ``done``.
        """
        for record in self:
            if record.waiver_id and record.waiver_id.state not in (
                "open",
                "done",
            ):
                error_message = """
...
Database ID: %s
Problem: Waiver '%s' is not in Open or Done status
Solution: Select a Waiver that has already reached Open or Done
""" % (
                    record._description,
                    record.id,
    ...
    def _05_check_amount_unallocated(self):
        """Reject opening a document with an unallocated remainder.

        :raises UserError: when ``amount_unallocated`` is not zero.
        """
        self.ensure_one()
...
""" % (
    ...
    def _30_mark_schedule_realized(self):
        """Flip every deducted Schedule line to state ``realized``.

        One Deduction line always realizes exactly one Schedule line
        (``_check_duplicate_schedule`` on the line model forbids the
        same Schedule appearing twice on one document), so each
        Schedule line's own ``amount_realized`` is simply that line's
        own ``amount``.
        ...
        self.ensure_one()
        for line in self.line_ids:
            line.schedule_id.write(
                {
                    "deduction_id": self.id,
                    "state": "realized",
                    "amount_realized": line.amount,
                }
            )
    ...
    def _get_policy_field(self):
        ...
        here too, or Odoo raises ``ValueError: Compute method failed
        to assign ...open_ok`` as soon as the field is read.
